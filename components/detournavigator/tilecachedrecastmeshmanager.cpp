...
namespace DetourNavigator
{
                                                   ...
                                                   const btTransform& transform, const AreaType areaType)
    {
        ...
        for (const auto& tilePosition : object->second)
        {
            ...
            if (tile != mTiles.end())
                result = tile->second.updateObject(id, shape, transform, areaType) || result;
        }
        lock.unlock();
        if (result)
            ++mRevision;
    ...
    bool TileCachedRecastMeshManager::addWater(const osg::Vec2i& cellPosition, const int cellSize, const btScalar level,
        ...
        getTilesPositions(cellSize, transform, mSettings, [&] (const TilePosition& tilePosition)
            {
                ...
                if (tile->second.addWater(cellPosition, cellSize, level, transform))
                {
                    lock.unlock();
                    tilesPositions.push_back(tilePosition);
                    result = true;
